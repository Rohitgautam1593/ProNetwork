...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-3">
        ...
        <button onclick="openAddUserModal()" class="bg-[#0A66C2] text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-[#004182] transition-all">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody id="admin-users-table" class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors group cursor-pointer" data-admin-preview data-preview-type="User" data-preview-id="<?php echo (int)$user['user_id']; ?>">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                    ...
                    <td class="px-6 py-4">
                        <select data-admin-ignore-preview onchange="updateUserRole(<?php echo $user['user_id']; ?>, this.value)" class="text-[12px] font-bold border-none bg-slate-50 rounded-lg px-2 py-1 focus:ring-0 cursor-pointer hover:bg-slate-100 transition-colors">
                        ...
                        <?php endif; ?>
                        <?php if(!empty($user['active_report_count'])): ?>
                            <span class="ml-2 px-2 py-1 bg-red-50 text-red-700 text-[10px] font-black rounded-lg border border-red-100"><?php echo (int)$user['active_report_count']; ?> active report<?php echo (int)$user['active_report_count'] === 1 ? '' : 's'; ?></span>
                        <?php endif; ?>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <?php if($user['status'] == 'Approved'): ?>
                            <span class="px-2 py-1 bg-green-50 text-green-600 text-[10px] font-bold rounded-lg border border-green-100">Approved</span>
                        ...
                        <?php else: ?>
                            <span class="px-2 py-1 bg-red-50 text-red-600 text-[10px] font-bold rounded-lg border border-red-100">Rejected</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <?php if($user['status'] == 'Pending'): ?>
                                <button data-admin-ignore-preview onclick="confirmApproveUser(<?php echo $user['user_id']; ?>)" title="Approve" class="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-all">
                                    <span class="material-symbols-outlined text-[20px]">check_circle</span>
                                </button>
                                <button data-admin-ignore-preview onclick="confirmRejectUser(<?php echo $user['user_id']; ?>)" title="Reject" class="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition-all">
                                    <span class="material-symbols-outlined text-[20px]">cancel</span>
                                </button>
                            <?php endif; ?>
                            <button data-admin-ignore-preview onclick="openEditUserModal(<?php echo htmlspecialchars(json_encode($user)); ?>)" title="Edit User" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                                <span class="material-symbols-outlined text-[20px]">edit</span>
                            </button>
                            <?php if($user['user_id'] != $_SESSION['user_id'] && !$user['is_admin']): ?>
                            <button data-admin-ignore-preview onclick="confirmDeleteUser(<?php echo $user['user_id']; ?>)" title="Delete User" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
...
<!-- Edit User Modal -->
<div id="editUserModal" class="fixed inset-0 z-[150] hidden overflow-y-auto">
    <!-- Backdrop -->
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closeEditUserModal()"></div>
    
    <!-- Scrolling centering container -->
    <div class="min-h-screen px-4 py-8 flex items-center justify-center">
        <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8 border border-slate-100 max-h-[calc(100vh-4rem)] overflow-y-auto custom-scrollbar">
...
<!-- Add User Modal -->
<div id="addUserModal" class="fixed inset-0 z-[150] hidden overflow-y-auto">
    <!-- Backdrop -->
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closeAddUserModal()"></div>
    
    <!-- Scrolling centering container -->
    <div class="min-h-screen px-4 py-8 flex items-center justify-center">
        <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8 border border-slate-100 max-h-[calc(100vh-4rem)] overflow-y-auto custom-scrollbar">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-slate-900 font-manrope">Add New User</h3>
                <button onclick="closeAddUserModal()" class="p-2 hover:bg-slate-50 rounded-full text-slate-400 transition-colors">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
            ...
            <form id="addUserForm" class="space-y-4">
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
                    <input type="text" id="add-full-name" required class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Email Address</label>
                    <input type="email" id="add-email" required class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Password</label>
                    <input type="password" id="add-password" required class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Platform Role</label>
                    <select id="add-role" class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                        <option value="Student">Student</option>
                ...
                <div class="pt-4 flex gap-3">
                    <button type="button" onclick="closeAddUserModal()" class="flex-1 py-3 bg-slate-50 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-100 transition-all">Cancel</button>
                    <button type="submit" class="flex-[2] py-3 bg-[#0A66C2] text-white rounded-xl text-sm font-bold hover:bg-[#004182] transition-all shadow-lg shadow-blue-500/20">Create User</button>
                </div>
            </form>
        </div>
    </div>
